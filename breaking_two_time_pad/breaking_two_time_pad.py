...
def string_to_hex(x):
    x = x.encode()
    return str(binascii.hexlify(x), 'ascii')

def hex_to_string(x):
    return str(binascii.unhexlify(x), 'ascii')
...
def crib_drag_all(crib: str, xor_messages: str) -> str:
    #TODO: not printing out all potential matches for each match
    for index in range(len(xor_messages)//2 - len(crib)):
        ...
        if matches:
            ...
            for index in range(len(matches)):
                if len(matches[index]) > 10:
                    matches[index] = matches[index][0:10]
                    matches[index].append("...")
            print(f"Suggested matches: {matches}")

def crib_drag_all_ciphers(crib: str, ciphers):
    for index1, index2 in itertools.combinations(range(len(ciphers)), 2):
        print(f"Ciphers {index1+1} and {index2+1}")
        try:
            xor_messages = xor_strings(ciphers[index1], ciphers[index2])
            crib_drag_all(crib, xor_messages)
        except:
            print("failed")
...
def main():
    ...
    crib_drag_all_ciphers(" the ", ciphertexts)
    #xor_messages = xor_strings(ciphertext_1, ciphertext_10)
    #print(xor_messages)
    #crib_drag_all(" the ", xor_messages)
